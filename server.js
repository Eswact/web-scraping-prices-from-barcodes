...
const CACHE_NULL = "__null__"; // Null price
const REDIS_LOG_PATH = path.join(__dirname, "redis-log.txt"); // Redis error logs
...
function logRedisError(context, err) {
    ...
    try {
        fs.appendFileSync(REDIS_LOG_PATH, line, "utf8");
    } catch (writeErr) {
        console.warn("[Redis] redis-log.txt yazılamadı:", writeErr.message);
    }
}
...
redis.on("error", (err) => logRedisError("Bağlantı / istemci hatası (redis error olayı)", err));

async function cacheGet(key) {
    try { return await redis.get(key); }
    catch (err) {
        logRedisError(`cacheGet okunamadı (key: ${key})`, err);
        return null;
    }
}
...
async function cacheSet(key, value) {
    try { await redis.setex(key, CACHE_TTL, value ?? CACHE_NULL); }
        ...
        logRedisError(`cacheSet yazılamadı (key: ${key})`, err);
    }
}
// Redis cache end
...
app.post("/api/search-stream-fast", async (req, res) => {
    ...
    try {
        ...
        // A tab is opened for each platform, and all platforms run in parallel independently.
        await Promise.all(
